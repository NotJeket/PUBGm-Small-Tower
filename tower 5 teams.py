...
def get_data():
    ...
    df = df.sort_values(by=["liveStateCount", "killNum"], ascending=[False, False]).head(5)
    df["background_image"] = df.apply(
        #apply test 2 in case of 2nd img \/
        lambda row: ("assets/test.png", "pulsing", "assets/test.png") if row["isOutsideBlueCircle"] else (
...
def update_table(n):
    ...
    for team_name, team_data in teams_sorted.head(5).iterrows():
        if all(live_state > 3 for live_state in team_data["liveState"]):
            ...
            logo = html.Td(html.Img(src=team_data["logo"], style={"height": "35px", "padding": "5px"}),
                           className="team-logo")
